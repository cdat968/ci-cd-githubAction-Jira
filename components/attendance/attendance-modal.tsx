...
export default function AttendanceModal({
    ...
    useEffect(() => {
        if (editRow) {
            ...
            setCheckIn(
                editRow.check_in_at
                    ? new Date(editRow.check_in_at).toLocaleTimeString(
                          "sv-SE",
                          {
                              hour: "2-digit",
                              minute: "2-digit",
            ...
            setCheckOut(
                editRow.check_out_at
                    ? new Date(editRow.check_out_at).toLocaleTimeString(
                          "sv-SE",
                          {
                              hour: "2-digit",
                              minute: "2-digit",
    ...
    async function handleSave() {
        ...
        if (!checkIn) {
            setError("Vui lòng nhập giờ vào.");
            return;
        }
        if (!note.trim()) {
            setError("Ghi chú bắt buộc cho chấm công thủ công.");
            return;
        }
